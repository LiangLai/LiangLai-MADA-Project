...
tidy_ph_test <- function(ph_obj, wave, model_label) {
  ...
  rownames(ph_df) <- NULL
  # Keep only the global test row and rename columns
  ph_df %>%
    as_tibble() %>%
    rename(p_value = p) %>%
    mutate(
...
# Model 3: Unadjusted categorical BMI variability
# Model 4: Adjusted categorical BMI variability

run_cox_models_by_wave <- function(data, wave) {
  
  # Define wave-specific exposure variable names
  bmi_cv_var   <- paste0("BMI_CV_W", wave)
  bmiv_cat_var <- paste0("BMIV_CAT_W", wave)
  
  #---------------------------------------
  # Model 1: Unadjusted continuous BMIV
  #---------------------------------------
  ...
  #---------------------------------------
  # Model 2: Adjusted continuous BMIV
  #---------------------------------------
  form_adj <- as.formula(
    paste0(
      "Surv(tstart, tstop, event_dementia) ~ ",
  ...
  #---------------------------------------
  # Model 3: Unadjusted categorical BMIV
  #---------------------------------------
  form_cat_unadj <- as.formula(
    paste0(
      "Surv(tstart, tstop, event_dementia) ~ ",
      bmiv_cat_var,
      " + cluster(HHIDPN)"
  ...
  fit_cat_unadj <- coxph(form_cat_unadj, data = data)
  ph_cat_unadj  <- cox.zph(fit_cat_unadj)
  ...
  tbl_cat_unadj <- tidy(
    ...
    mutate(
      wave = paste0("W", wave),
      model = "Categorical_Unadjusted"
    )
  
  ph_tbl_cat_unadj <- tidy_ph_test(ph_cat_unadj, wave, "Categorical_Unadjusted")
  
  #---------------------------------------
  # Model 4: Adjusted categorical BMIV
  #---------------------------------------
  form_cat_adj <- as.formula(
    paste0(
      "Surv(tstart, tstop, event_dementia) ~ ",
      bmiv_cat_var,
      " + BASELINE_AGE + RAGENDER + RACE_ETH3 + RAEDUC + ",
  ...
  fit_cat_adj <- coxph(form_cat_adj, data = data)
  ph_cat_adj  <- cox.zph(fit_cat_adj)
  ...
  ph_tbl_cat_adj <- tidy_ph_test(ph_cat_adj, wave, "Categorical_Adjusted")
  
  #---------------------------------------
  # Return all fitted models and tidy outputs
  #---------------------------------------
  list(
    # fitted model objects
    fit_unadj = fit_unadj,
    fit_adj = fit_adj,
    fit_cat_unadj = fit_cat_unadj,
    fit_cat_adj = fit_cat_adj,
    
    # tidy regression tables
    tbl_unadj = tbl_unadj,
    tbl_adj = tbl_adj,
    tbl_cat_unadj = tbl_cat_unadj,
    tbl_cat_adj = tbl_cat_adj,
    
    # proportional hazards test tables
    ph_unadj = ph_tbl_unadj,
    ph_adj = ph_tbl_adj,
    ph_cat_unadj = ph_tbl_cat_unadj,
    ph_cat_adj = ph_tbl_cat_adj
  )
}
